...
def main():
    ...
    p.add_argument("--model-out", default="outputs/image_only.joblib")
    args = p.parse_args()

    cfg = load_config(args.config)
    ds = SentiMMDLikeDataset(args.train_jsonl)
    flags = ModuleFlags(text=False, image=True, fusion=False, kb=False, aggregator=False)

    model = SentiMMPipeline(cfg, flags)
    model.fit(ds.texts(), ds.image_paths(), ds.video_paths(), ds.kb_texts(), ds.labels())
    model.save(args.model_out)

    log = {"mode": "image_only", "train_size": len(ds.samples), "model_out": args.model_out}
    Path("outputs/train_image_only_log.json").write_text(json.dumps(log, indent=2), encoding="utf-8")
    print(log)
